...
public class AnimFrameDrawable extends Drawable
        implements AnimFrameLoader.FrameCallback, Animatable, Animatable2Compat {
    ...
    /** not stop in background */
    private boolean enableBackgroundRunning;
    /**
    ...
    AnimFrameDrawable(FrameState state) {
    ...
    public void setFrameTransformation(
            Transformation<Frame> frameTransformation, Frame firstFrame) {
        state.frameLoader.setFrameTransformation(frameTransformation, firstFrame);
    }

    public Transformation<Frame> getFrameTransformation() {
    ...
    public void enableBackgroundRunning(boolean enable) {
        this.enableBackgroundRunning = enable;
    }
    ...
    public void draw(@NonNull Canvas canvas) {
        ...
        if (maxLoopCount != LOOP_FOREVER && loopCount >= maxLoopCount) {
            if (placeholderBitmap == null && placeholderDrawable == null) {
                decodePlaceholder();
            }
            if (placeholderBitmap != null) {
                canvas.drawBitmap(placeholderBitmap, null, getBounds(), getPaint());
            } else if (placeholderDrawable != null) {
                placeholderDrawable.setBounds(getBounds());
                placeholderDrawable.draw(canvas);
            } else {
                Frame currentFrame = state.frameLoader.getCurrentFrame();
...
//            invalidateFrame();
    ...
    private void decodePlaceholder() {
        if (getFrameIndex() == getFrameCount() - 1) {
            Frame.IFrame currentFrame = state.frameLoader.getCurrentFrame().getFrameSource();
            if (currentFrame instanceof Frame.FrameBitmap) {
                Bitmap bitmap = ((Frame.FrameBitmap)currentFrame).getBitmap();
                placeholderBitmap = Bitmap.createBitmap(bitmap);
            } else if (currentFrame instanceof Frame.FrameDrawable){
                Drawable drawable = ((Frame.FrameDrawable)currentFrame).getDrawable();
                placeholderDrawable = drawable;
                Log.d("luo", "decodePlaceholder: " + getFrameIndex());
            }
        }
    }
     ...
     * {@link AnimFrameDrawable#setLoopCount(int)}.
     *
     ...
     *
     * @see AnimFrameDrawable#unregisterAnimationCallback(AnimationCallback).
    ...
    static final class FrameState extends ConstantState {
        @VisibleForTesting
        final AnimFrameLoader frameLoader;

        FrameState(AnimFrameLoader frameLoader) {
        ...
        public Drawable newDrawable() {
            return new AnimFrameDrawable(this);
        }
